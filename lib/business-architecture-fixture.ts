...
export const blmSections: ArchitectureSection[] = [
  ...
  },
  {
    ...
    summaryZh: "不是上链存证，而是 AI Agent + 智能合约共同驱动的证据到放款闭环。",
    summaryEn: "Not on-chain notarization, but an AI-agent + smart-contract evidence-to-disbursement loop.",
    pointsZh: ["Evidence Agent 抽取和分类单证。", "Gate Agent 判断签章、物流、质检、买家验收是否满足。", "智能合约把多方确认转成共享状态机，减少反复邮件确认、Excel 对账和人工复核。", "Risk Agent 生成 Readiness Score、Financing Pack 和资金方 memo。"],
    pointsEn: ["Evidence Agent extracts and classifies documents.", "Gate Agent checks whether signing, logistics, QC, and buyer acceptance gates are satisfied.", "Smart contracts turn multi-party confirmations into a shared state machine, reducing repeated email confirmation, Excel reconciliation, and manual re-checking.", "Risk Agent generates Readiness Score, Financing Pack, and financier memo."],
  },
  {
    id: "business-design",
...
export const valueChainSections: ArchitectureSection[] = [
  ...
  },
  {
    ...
    summaryZh: "ChainTrace 的核心增值点是把证据链转化为 Readiness Score、Financing Pack 和 Financier Memo，并通过合约把融资条件变成可执行 gate。",
    summaryEn: "ChainTrace's core added value is converting evidence chains into Readiness Score, Financing Pack, and Financier Memo, while contracts turn financing conditions into executable gates.",
    pointsZh: ["资金方不需要人工翻 PDF，而是看风险 flags、缺口、前置条件和 machineDecision。", "AI Agent 替代预审助理和风控文书工作。", "ReceivableLoan 自动读取签章和物流 registry；gate 不通过时不能放款，减少放款前人工复核摩擦。", "API 让银行、保理商、Agent 和平台后台直接消费同一份判断。"],
    pointsEn: ["Financiers do not manually read PDFs; they review risk flags, gaps, approval conditions, and machineDecision.", "AI agents replace pre-review assistants and risk-documentation work.", "ReceivableLoan reads signing and logistics registries automatically; if gates do not pass, disbursement cannot happen, reducing pre-disbursement manual re-check friction.", "The API lets banks, factors, agents, and platform back offices consume the same decision."],
  },
  {
...
export const technicalArchitecture: ArchitectureLayer[] = [
  {
    ...
    capabilitiesZh: ["业务工作台", "融资评分", "Agent 工作台", "合约控制台", "资金方视图", "专业审查视图"],
    capabilitiesEn: ["Business workspace", "Readiness report", "Agent workbench", "Contract console", "Financier view", "Professional review view"],
  },
  {
    id: "api",
    titleZh: "API 层",
    ...
    capabilitiesZh: ["/api/financing-pack", "未来 /api/trades", "未来 /api/evidence", "未来 /api/agents", "未来 /api/professional-review", "未来 /api/loans"],
    capabilitiesEn: ["/api/financing-pack", "future /api/trades", "future /api/evidence", "future /api/agents", "future /api/professional-review", "future /api/loans"],
  },
  {
    id: "agent-services",
  ...
  },
  {
    ...
    capabilitiesZh: ["TradeSigningRegistry", "LogisticsEvidenceRegistry", "FinancierPool", "BankVault", "ReceivableLoan", "Automatic gate enforcement"],
    capabilitiesEn: ["TradeSigningRegistry", "LogisticsEvidenceRegistry", "FinancierPool", "BankVault", "ReceivableLoan", "Automatic gate enforcement"],
  },
  {
    id: "storage",
    titleZh: "存储层",
    ...
    capabilitiesZh: ["贸易主数据", "文档 metadata", "原文文件", "Agent 输出", "专业机构决策", "合约事件", "审计日志"],
    capabilitiesEn: ["Trade master data", "document metadata", "raw files", "agent outputs", "professional decisions", "contract events", "audit logs"],
  },
  {
    id: "devops",
