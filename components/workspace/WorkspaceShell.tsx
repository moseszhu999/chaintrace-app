      ...
      <section className="panel" style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "230px minmax(0, 1fr)", minHeight: 760 }}>
          <aside style={{ borderRight: "1px solid var(--border)", padding: 22, background: "rgba(255,250,240,.72)" }}>
            <Link href="/" style={{ display: "flex", alignItems: "center", gap: 10, fontWeight: 950, marginBottom: 24 }}>
            ...
            <nav style={{ display: "grid", gap: 8 }} aria-label="Workspace navigation">
              {workspaceNavItems.map((item) => (
                <Link key={item.key} href={item.href} className={active === item.key ? "primary-button" : "secondary-button"} style={{ justifyContent: "flex-start", width: "100%" }}>
                  {zh ? item.zh : item.en}
                </Link>
              ))}
            </nav>
            ...
            <Link className="secondary-button" href="/login" style={{ width: "100%" }}>{t(zh, "退出模拟登录", "Exit mock login")}</Link>
          </aside>
