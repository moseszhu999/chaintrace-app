(function () {
  ...
  var data = {
    ...
    "buyer-dashboard": { title: "Agent 买方确认任务分发", did: ["从案件生成买方确认任务", "整理 PO、发票和到期日", "标记需要买方确认的付款义务"], wait: ["等待买方打开确认任务"], human: ["买方决定接受、部分接受或提出争议"] },
    "buyer-invoice-review": { title: "Agent 发票核验辅助", did: ["对齐 PO 与发票金额", "标记 USD 8,000 差异", "展示出口商差异说明", "准备买方决策草稿"], wait: ["等待买方确认是否接受发票"], human: ["买方对发票接受状态负责"] },
    "buyer-receipt-confirm": { title: "Agent 收货与义务确认辅助", did: ["合并物流证明", "合并质检结论", "准备付款到期日字段", "生成 acknowledgement 草稿"], wait: ["等待买方确认收货和付款义务"], human: ["买方确认或提出 dispute"] },
    "buyer-ack-result": { title: "Agent 买方确认结果交接", did: ["把买方确认转成事实节点", "同步银行可审摘要", "同步资金方 obligation proof"], wait: ["等待后续审核流程读取"], human: ["买方保留付款义务确认责任"] },
