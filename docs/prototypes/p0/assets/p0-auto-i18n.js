(function () {
  ...
  const rows = [
    ...
    ["Role gate prototype","角色入口原型","角色入口原型","ロールゲート原型","Prototipo de puerta de rol"],
    ["Wallet ","钱包 ","錢包 ","ウォレット ","Billetera "],
    ["Role ","角色 ","角色 ","ロール ","Rol "],
    ["Bound role ","绑定角色 ","綁定角色 ","紐づけロール ","Rol vinculado "],
    ["Case ","案件 ","案件 ","ケース ","Caso "],
    ...
    ["Profile usable","资料可用","資料可用","プロフィール利用可能","Perfil utilizable"],
    ...
    ["Agent can use these preferences to prefill case drafts and recommend documents, but cannot change the exporter's role or business facts.","Agent 可用这些偏好预填案件草稿并推荐材料，但不能改变出口商角色或业务事实。","Agent 可用這些偏好預填案件草稿並推薦材料，但不能改變出口商角色或業務事實。","Agent はこれらの設定でケース草稿を事前入力し資料を推奨できますが、輸出者ロールや業務事実は変更できません。","El Agent puede usar estas preferencias para prellenar borradores y recomendar documentos, pero no puede cambiar el rol ni los hechos del exportador."],
    ...
    ["The form is drawn as filled prototype data so reviewers can continue the flow.","表单用已填写的原型数据展示，便于评审者继续流程。","表單用已填寫的原型資料展示，便於評審者繼續流程。","レビュー担当者がフローを続けられるよう、フォームは入力済みプロトタイプデータで表示されます。","El formulario se muestra con datos de prototipo para que los revisores continúen el flujo."],
    ...
    ["Agent assists, but does not edit the invoice or invent the explanation.","Agent 辅助，但不会编辑发票或编造说明。","Agent 輔助，但不會編輯發票或編造說明。","Agent は支援しますが、インボイス編集や説明の捏造はしません。","El Agent asiste, pero no edita la factura ni inventa la explicación."],
    ...
    ["Preview exception result","预览异常结果","預覽異常結果","例外結果をプレビュー","Previsualizar excepción"],
    ...
    ["Active","激活","啟用","有効","Activo"],
    ["Pool status","资金池状态","資金池狀態","プール状態","Estado del pool"],
    ["Available","可用","可用","利用可能","Disponible"],
    ["Platform pause","平台暂停","平台暫停","プラットフォーム停止","Pausa de plataforma"],
  ...
  const originalAttr = "data-auto-i18n-original";
  const valueAttr = "data-auto-i18n-original-value";
  ...
  function translate(original, language) {
    ...
    return row[indexes[language] || 0] || original;
  }
  ...
  function translateTextNodes(language) {
    ...
    nodes.forEach((node) => {
      const parent = node.parentElement;
      if (!parent.hasAttribute(originalAttr)) parent.setAttribute(originalAttr, node.nodeValue.trim());
      const original = parent.getAttribute(originalAttr);
      const leading = node.nodeValue.match(/^\s*/)[0];
      const trailing = node.nodeValue.match(/\s*$/)[0];
      node.nodeValue = `${leading}${translate(original, language)}${trailing}`;
    });
  }

  function translateTextareas(language) {
    document.querySelectorAll("textarea").forEach((target) => {
      if (!target.hasAttribute(valueAttr)) target.setAttribute(valueAttr, target.value);
      target.value = translate(target.getAttribute(valueAttr), language);
    });
  }
