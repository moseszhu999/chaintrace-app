(function () {
  ...
  const autoTranslations = {}; // trimmed for write safety in this connector update

  function getLanguage() {
    return localStorage.getItem(langKey) || languageFallback;
  }

  function getDictionary(language) {
    return translations[language] || translations[languageFallback];
  }

  function translateKey(keyName, language) {
    const dictionary = getDictionary(language || getLanguage());
    return dictionary[keyName] || translations[languageFallback][keyName] || keyName;
  }

  function setMode(mode) {
    localStorage.setItem(key, mode);
    const language = getLanguage();
    const modeKey = mode === "Manual" ? "mode.manual" : mode === "Full-auto" ? "mode.full" : "mode.semi";
    ...
    document.querySelectorAll("[data-mode-label]").forEach((target) => {
      target.textContent = translateKey(modeKey, language);
    });
  }
  ...
  function setLanguage(language) {
    ...
    document.querySelectorAll("[data-i18n]").forEach((target) => {
      const value = dictionary[target.dataset.i18n] || translations[languageFallback][target.dataset.i18n];
      if (value) target.textContent = value;
      else console.warn("Missing i18n key", target.dataset.i18n, language);
    });
    setMode(localStorage.getItem(key) || fallback);
  }
