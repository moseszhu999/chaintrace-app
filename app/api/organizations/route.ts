...
import { safeCreateV2Organization, safeListV2OrganizationsForUser } from "@/lib/repositories/safe-v2-organization-repository";
...
export async function GET(request: Request) {
  try {
    ...
    const organizations = await safeListV2OrganizationsForUser(identity.email);
...
export async function POST(request: Request) {
  try {
    ...
    const result = await safeCreateV2Organization({
